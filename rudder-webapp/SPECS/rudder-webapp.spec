...
%define apache_errlog_file_ssl %{rudderlogdir}/%{apache}/ssl-error.log
%define apache_log_file_ssl    %{rudderlogdir}/%{apache}/ssl-access.log
...
sed -i 's@%APACHE_ERRLOG_FILE%@%{apache_errlog_file}@' %{_sourcedir}/rudder-sources/rudder/rudder-web/src/main/resources/apache2-non-ssl.conf
sed -i 's@%APACHE_LOG_FILE%@%{apache_log_file}@'       %{_sourcedir}/rudder-sources/rudder/rudder-web/src/main/resources/apache2-non-ssl.conf

sed -i 's@%APACHE_ERRLOG_FILE%@%{apache_errlog_file_ssl}@' %{_sourcedir}/rudder-sources/rudder/rudder-web/src/main/resources/apache2-ssl.conf
sed -i 's@%APACHE_LOG_FILE%@%{apache_log_file_ssl}@'       %{_sourcedir}/rudder-sources/rudder/rudder-web/src/main/resources/apache2-ssl.conf
...
mkdir -p %{buildroot}%{rudderdir}/etc/apache2/
...
cp %{_sourcedir}/rudder-sources/rudder/rudder-web/src/main/resources/apache2-non-ssl.conf %{buildroot}/etc/%{apache_vhost_dir}/rudder-non-ssl.conf
cp %{_sourcedir}/rudder-sources/rudder/rudder-web/src/main/resources/apache2-ssl.conf %{buildroot}/etc/%{apache_vhost_dir}/rudder-ssl.conf
cp %{_sourcedir}/rudder-sources/rudder/rudder-web/src/main/resources/apache2-config.conf %{buildroot}%{rudderdir}/etc/apache2/rudder-config.conf
...
# If the old apache vhost is detected, move it out of the way
for i in /etc/apache2/vhosts.d/rudder-default /etc/httpd/conf.d/rudder-default; do
	if [ -f "${i}" ]; then
		echo -n "INFO: Moving old apache vhost ( ${i} ) to /opt/rudder/etc/apache2/rudder-old-vhost.bak..."
		mv ${i} /opt/rudder/etc/apache2/rudder-old-vhost.bak
		echo " Done"
	fi
done

# If there is no autogenerated certificate for Rudder, create one
if [ ! -f /opt/rudder/etc/apache2/rudder.crt ]; then
	echo -n "INFO: No Rudder SSL certificate detected. Creating one..."
	openssl req -batch -x509 -nodes -days 1825 -newkey rsa:2048 -keyout /opt/rudder/etc/apache2/rudder.key -out /opt/rudder/etc/apache2/rudder.crt > /dev/null 2>&1
	echo " Done"
fi
...
%config(noreplace) /etc/%{apache_vhost_dir}/rudder-non-ssl.conf
%config(noreplace) /etc/%{apache_vhost_dir}/rudder-ssl.conf
%config(noreplace) %{buildroot}%{rudderdir}/etc/apache2/rudder-config.conf
